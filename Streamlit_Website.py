...
if page == "Video Page":
    ...
    if st.button("Start Video"):
        # URL to the video on GitHub (ensure it's a direct link to the video file)
        video_url = "https://github.com/Alamyy/ProWorth/raw/refs/heads/main/Football%20in%20slow%20motion%20-%20social%20media%20video%20ad%20-%20stock%20video.mp4"  # Video URL
        st.video(video_url)
    else:
        st.info("Press the 'Start Video' button to watch the video.")
...
if page == "Player Analyzer":
    ...
    if player_name_input != "-- Select a Player --":
        ...
        if not player_data.empty:
            ...
            st.markdown("---")
            st.markdown(f"### **{player_name} Details**")
            st.image(player_info['image_url'], width=250)
            st.write(f"**Position**: {player_info['position']}")
            st.write(f"**Country**: {player_info['country']}")
            st.write(f"**Current Club**: {player_info['current_club_name']}")
        else:
            st.warning(f"No data available for player: {player_name_input}")

# ---------------------- Top Market Values 2026 ----------------------
if page == "Top Market Values 2026":
    st.markdown("<h1 style='text-align: center; color: #2E86C1;'>Top Market Values 2026</h1>", unsafe_allow_html=True)

    top_players = df[['name_x', 'predicted_value_2026']].sort_values(by='predicted_value_2026', ascending=False).head(10)
